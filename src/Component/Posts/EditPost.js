import React, { useContext, useEffect, useState } from 'react'
import { useNavigate, useParams } from 'react-router-dom'
import {BiArrowBack} from 'react-icons/bi'

export function EditPost() {

    const params = useParams()
    const [state, setState] = useState(null)  
    const navigate = useNavigate()

    useEffect(() => { 
        try{
                fetch(`http://localhost:3000/posts/${params.id}`,{
                    method: 'Get',
                })
               .then(response => response.json())
               .then((data) => {
                 setState(data)
               })
               .catch(error => console.log(error))
    
          }catch(e){
              console.log(e)
          }
    },[])

    function handleChange(e){
          let obj = state
          obj[`${e.target.name}`] = e.target.value
          obj.at.date = `${(new Date()).getDate()}/${(new Date()).getMonth()}/${(new Date()).getFullYear()}`
          obj.at.time = `${(new Date()).getHours()}:${(new Date()).getMinutes()}`
          setState(obj)
          console.log(obj)
    }

    function handleSubmit(e){
      e.preventDefault()

      try{
        fetch(`http://localhost:3000/posts/${params.id}`,{
            method: 'Put',
            body:JSON.stringify(state),
            headers :{
                'Content-type' : 'Application/json'
            }
        })
        .then(response => response.json())
        .then((data) => {
            console.log(data)
            navigate('/posts')
        })
        .catch(error => console.log(error))

      }catch(e){
          console.log(e)
      }
    } 

    function Display(){
      return(
            <>
            <div key={state.UserID} className="col-12 p-3 card">
                <div className="row g-3">
                        <div className="col-12">
                            <h1>What's to change !</h1>
                        </div>
                        <div className="col-12 d-flex flex-row justify-content-start align-items-center">
                            <label>Post</label>&emsp;
                            <input  type="text" readOnly disabled value={params.id}
                                    maxLength={15} className="form-control w-25" required/>
                        </div>
                        <div className="mb-3">
                            <input type="text" maxLength={20} className="form-control" id="exampleFormControlInput1" name="title" placeholder={state.title} onChange={handleChange} />
                        </div>
                        <div className="mb-3">
                            <textarea className="form-control" maxLength={100} id="exampleFormControlTextarea1" rows="3" name="post" placeholder={state.post} onChange={handleChange}></textarea>
                        </div>
                        <div className="col-12">
                            <button type="submit" className="btn btn-primary" onClick={handleSubmit}>Post</button>&emsp;
                            <button className="btn btn-primary" onClick={() => navigate(-1)} ><BiArrowBack /></button>
                        </div>
                </div>
            </div>     
            </>
        )
    }

    return (
    <div className='container'> 
      <div className='row mt-5'>
        {
          state !== null ? Display() : <h1>Data is Being Fetched...</h1>
        }
      </div>
    </div>
  )
}
